...
{
    [Fact]
    public async Task Handle_whenAlreadyEnrolled_ReturnDuplicateError()
    {
        ...
        //Assert on the machine-readable Code (the constract) plus full record equality
        //Not on the human-readable message see M7 sealed-record pattern.

        Assert.False(result.IsSuccess);
        Assert.Equal("already_enrolled", result.Error.Code);
        Assert.Equal(EnrollmentError.AlreadyEnrolled(99, "CS-401"), result.Error);

        //The duplicate branch never write -prove it.
    ...
    public async Task Handle_whenCourseFull_ReturnsCapacityError()
    {
        ...
        var course =new Course
        {
            ...
            Title="Advanced Web Dev",
            MaxCapacity=35,
            Enrollments=Enumerable.Range(1,35).Select(i=>new Enrollment {Id=i, CourseId=1,Status=0}).ToList()
        };
        ...
        //Assert: typed error matchines the M7 sealed-record factory
    ...
    public async Task Handle_SuccessfulPath_AddsEnrollmentOnce()
    {
        ...
        var course=new Course
        {
            ...
            Title="Advanced Web Dev",
            MaxCapacity=35,
            Enrollments=Enumerable.Range(1, 20).Select(i=> new Enrollment{Id=i, CourseId=1,Status=0,}).ToList(),
        };
        ...
        var result =await handler.Handle(command, CancellationToken.None);
         ...
         await enrollmentService.Received(1)
         .AddAsync(Arg.Is<Enrollment>(e=>e.StudentId==100 && e.CourseId==1), Arg.Any<CancellationToken>());
    }
}